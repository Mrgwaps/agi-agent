...
type RightPanel = 'cost' | 'earnings' | 'skills' | null;
  ...
  return (
    <header className="flex-shrink-0 h-12 border-b border-border flex items-center px-4 gap-3 bg-card z-10">
      <div className="flex items-center gap-2 select-none flex-shrink-0">
        ...
        <span className="text-sm font-bold text-text tracking-tight hidden sm:block">AGI Agent</span>
...
export default function AppClient() {
  ...
  function handleTaskCreated(_taskId: string) {
  ...
  return (
    <div className="neural-bg flex flex-col h-screen overflow-hidden">
      <Header
        ...
        avatarSlot={avatarActive ? <VoiceAvatar enabled={avatarActive} /> : null}
      />
          ...
          {rightPanel === 'cost' && (
            ...
            >
              <div className="w-60 h-full overflow-y-auto">
                ...
                <CostTracker taskId={currentTaskId} onOpenSettings={() => setSettingsOpen(true)} />
              </div>
            </motion.aside>
          )}
          {rightPanel === 'earnings' && (
          ...
          {rightPanel === 'skills' && (
            ...
            >
              <div className="h-full" style={{ width: 280 }}>
                <SkillsPanel pollInterval={60_000} />
              </div>
            </motion.aside>
          )}
        </AnimatePresence>
