...
class PlannerService:
    ...
    async def create_plan(
        ...
        try:
            ...
            # Safety net: if LLM returned empty plan, use domain fallback
            if not steps:
                logger.warning("LLM returned empty plan, using fallback for goal: %s", goal[:80])
...
Keep plans concise: 1-3 steps for most tasks, max 5 for complex ones.
Prefer llm_only for coding, writing and analysis. Use web tools only when fresh data is needed.
...
- llm_only: reasoning, analysis, synthesis, coding that doesn't need external tools
